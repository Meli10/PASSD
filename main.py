...
# these are lists of initialized characters
	...
	def __init__(self, root, geo, title) -> None:
		...
		Label(self.root, text='Your Password', font=14).grid(
			row=0, column=0, padx=10, pady=10)
		Label(self.root, text='Username', font=14).grid(
			row=1, column=0, padx=10, pady=10)
		Label(self.root, text='Website', font=14).grid(row=2, column=0, padx=10, pady=10)
		...
		btn = ttk.Button(self.root, text="Generate", padding=5, style='success Outline.TButton', width=20, command=self.generate).grid(row=1, column=2)
		...
		ttk.Button(self.root, text='Show All', width=20, padding=5, command=self.view).grid(row=3, column=0, padx=6)
		
		ttk.Button(self.root, text='Update', width=20, padding=5, command=self.update).grid(row=3, column=1)

		# ========self.tree view=============

		self.tree = ttk.Treeview(self.root, height=17)
		...
		self.tree.column('pas', width=190, anchor=W)
		self.tree.heading('#0', text='')
		self.tree.heading('site', text='Website', anchor=W)
		self.tree.heading('user', text='Username', anchor=W)
		self.tree.heading('pas', text='Password', anchor=W)
		self.tree.grid(row=4, column=0, columnspan=3, pady=12, padx=2)
	...
	def copy(self):
		...
		Window.clipboard_append(self.sit.get(), self.user_id.get(), self.pa.get)
	...
	def save(self):
		...
		info = self.tree.insert(parent='', index='end', text='', values=(self.site.get(), self.user_id.get(), self.pa.get()))
  
	def generate(self):
		# this function will produce a random string which
		# will be used as password
		if self.length.get() == 'Set password length':
			messagebox.showerror('Attention!', "Please set a password length")
		else:	
			a = ''
	...
	def export(self):
		...
		def exp(x):
			with open(f'{x}.csv', 'w', newline='') as f:
				chompa = csv.writer(f, dialect='excel')
				for r in back.show():
					chompa.writerow(r)
			messagebox.showinfo("File Saved", f"Saved as {x}.csv")


if __name__ == '__main__':
	win = Style(theme='vapor').master
	name = 'PASSD'
	dimension = '618x482+600+200'

app = window(win, dimension, name)
win.iconbitmap("lock.ico")
win.mainloop()
